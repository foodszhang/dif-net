...
if __name__ == "__main__":
    ...
    # -- initialize model
    model = DIF_Net(num_views=args.num_views, combine=args.combine).cuda()

    # -- initialize optimizer, lr scheduler, and loss function
    optimizer = torch.optim.SGD(
        model.parameters(), lr=args.lr, momentum=0.98, weight_decay=1e-3
    )
    lr_scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=1, gamma=np.power(0.001, 1 / args.epoch)
    )
    ...
    for epoch in range(args.epoch + 1):
        ...
        if epoch % 50 == 0 or (epoch >= (args.epoch - 100) and epoch % 10 == 0):
            metrics, _ = eval_one_epoch(model, eval_loader, args.eval_npoint)
            msg = f" --- epoch {epoch}"
